...
from utils.labels import LabelInfo, ensure_label, sanitize_label
...
class ErrorPatternAnalyzer:
    ...
    def __init__(self, base_dir: Path | str = ".", prompt_path: Path | str = "prompts/pattern_analysis/error_pattern_analysis.md") -> None:
        load_dotenv()
    ...
    def analyze(self, languages: Iterable[str] | None = None, *, label: LabelInfo | str) -> List[str]:
        label_info = ensure_label(label)

        languages_to_process = list(languages or self._discover_languages(label_info))
        if not languages_to_process:
            print(f"⚠️  No aggregated issues found for label '{label_info.original}'.")
            return []
    ...
    def _call_model(self, language: str, category: str, severity: str, issues: List[Dict]) -> Optional[Dict]:
    ...
    def _write_language_report(self, language: str, report: Dict, label: LabelInfo) -> None:
    ...
    def _load_combined_issues(self, language: str, label: LabelInfo) -> Optional[Dict[str, List[Dict]]]:
        ...
        print(f"⏭️  Skipping {language} - run issue aggregation for label '{label.original}' first")
        return None

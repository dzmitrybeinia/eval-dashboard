...
class LZLessonConverter(BaseConverter):
    """Convert LZ lesson content to markdown, excluding all quiz/exercise types."""
    ...
    def _slides_to_markdown(self, slides: List[Dict], md_lines: List[str], sections: List[Dict] | None) -> None:
        ...
        for slide_num, slide in enumerate(slides):
            ...
            any_quiz_objects = [
                obj for obj in generated_objects if obj.get("type")
            ]

            if not any_quiz_objects:
                continue

            first_quiz_inputs = any_quiz_objects[0].get("inputs", {})
